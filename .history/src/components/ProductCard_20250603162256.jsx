 ...
 * @param {string} props.product.image - 模型/图片路径
 * @param {Function} props.onOpenModal - 打开模态框的回调
 */
function ProductCard({ product, onOpenModal }) {
  const [tilt, setTilt] = useState({ x: 0, y: 0, hover: false });
  const [isMobile, setIsMobile] = useState(window.innerWidth <= 780);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isVisible, setIsVisible] = useState(false); // 是否首次进入视口
  ...
  // IntersectionObserver：当卡片进入视口时，将 isVisible 设为 true
  useEffect(() => {
