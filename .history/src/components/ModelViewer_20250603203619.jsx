...
function ModelViewer({ fileUrl }) {
  ...
  // 3. 切换全屏：先渲染 Portal 的 <div>，然后把 Canvas 移进去
  useEffect(() => {
    const renderer = rendererRef.current;
    if (!renderer) return;

    // 下一帧保证 React 已渲染出 fullscreenContainerRef.current
    setTimeout(() => {
      const canvas = renderer.domElement;
      if (isFullscreen) {
        // 如果 Canvas 当前在 normal 容器里，则移到 fullscreen 容器
        if (
          normalContainerRef.current &&
        ...
        // 退出全屏，将 Canvas 从 fullscreen 容器移回 normal
        if (
          fullscreenContainerRef.current &&
          canvas.parentNode === fullscreenContainerRef.current &&
          normalContainerRef.current
  ...
  return (
    <>
      ...
      >
        ...
        {/* Three.js 将自动插入 <canvas> */}
      </div>

      {/* 全屏模式时，通过 Portal 将空容器渲染到 document.body */}
      {isFullscreen &&
